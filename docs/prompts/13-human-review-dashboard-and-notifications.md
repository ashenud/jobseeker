...
Run this prompt as fresh context step **13**. After this prompt is complete, committed, continue to the next prompt sequentially.
...
Acceptance is through real HTTP/browser behavior against the Docker Compose stack,
not by calling private methods on an in-process fake app. The E2E must persist a
job, display its score/evidence/draft, save a new revision, approve to a package,
skip another job with a reason, restart the API, and verify the records remain.
...
8. Run the milestone acceptance matrix, required Docker integration or live smoke, repository fast suite, and clean-checkout reproduction. Generate `artifacts/verification/milestone-13.json` from the real results.
...
10. If checks pass, update required project records, commit with `milestone-13: <result>`, and continue to the next prompt sequentially unless this is prompt 22.
...
- [ ] `artifacts/verification/milestone-13.json` identifies the tested commit, commands, exit codes, test counts, acceptance IDs, and reviewer verdict.
