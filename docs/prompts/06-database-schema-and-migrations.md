...
Run this prompt as fresh context step **06**. After this prompt is complete, committed, continue to the next prompt sequentially.
...
8. Run the milestone acceptance matrix, required Docker integration or live smoke, repository fast suite, and clean-checkout reproduction. Generate `artifacts/verification/milestone-06.json` from the real results.
...
10. If checks pass, update required project records, commit with `milestone-06: <result>`, and continue to the next prompt sequentially unless this is prompt 22.
...
- [ ] `artifacts/verification/milestone-06.json` identifies the tested commit, commands, exit codes, test counts, acceptance IDs, and reviewer verdict.
