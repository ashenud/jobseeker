...
default_subnet() {
    case "$run_label" in
        preflight) printf '10.254.251.0/24' ;;
        main) printf '10.254.252.0/24' ;;
        clean) printf '10.254.253.0/24' ;;
    esac
}

main() {
    output_dir="${M05_OUTPUT_DIR:-artifacts/verification/milestone-05-preflight}"
    run_label="${M05_RUN_LABEL:-preflight}"
    project_name="${COMPOSE_PROJECT_NAME:-jobseeker_m05_preflight}"
    reference_dir=""
    ...
    run_gate 06-http-contracts no \
        docker compose --profile dev exec -T api python scripts/check_architecture_http.py \
        --base-url http://127.0.0.1:8000 || return $?
    run_gate 07-focused-pytest yes \
        ...
        tests/test_milestone_05_http.py || return $?
    run_gate 08-policy-pytest yes \
        docker compose --profile dev run --rm --no-deps api pytest -q \
        tests/test_policy.py tests/test_milestone_05_providers.py || return $?
    run_gate 09-migrate-up no \
